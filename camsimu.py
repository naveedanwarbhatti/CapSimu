...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Simulate capacitor charging and discharging with checkpointing."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="220e-6",
        help="Comma-separated capacitance values in farads (default: '220e-6')",
    )
    parser.add_argument(
        "--r_charging",
        type=float,
        default=1000,
        help="Resistance during charging in ohms (default: 1000 Ω)",
    )
    parser.add_argument(
        "--v_start",
        type=float,
        default=3.3,
        help="Startup voltage of MCU (default: 3.3 V)",
    )
    parser.add_argument(
        "--v_cutoff",
        type=float,
        default=1.8,
        help="Minimum operating voltage of MCU (default: 1.8 V)",
    )
    parser.add_argument(
        "--total_cycles_required",
        type=int,
        default=100000,
        help="Total cycles required by the application (default: 100000 cycles)",
    )
    parser.add_argument(
        "--total_checkpoint_size",
        type=int,
        default=128,
        help="Total bytes required by the checkpoint in bytes (default: 128 bytes)",
    )
    parser.add_argument(
        "--total_restore_size",
        type=int,
        default=128,
        help="Total bytes required by the restore in bytes (default: 128 bytes)",
    )
    parser.add_argument(
        "--trace",
        type=str,
        default="CapSimu/traces/RF_2.csv",
        help="CSV file path for voltage values (default: '\traces\RF_2.csv')",
    )
    parser.add_argument(
        "--mcu",
        type=str,
        default="stm32l152re_32Mhz",
        help="MCU name for the experiment (e.g., stm32l152re_32Mhz)",
    )
    parser.add_argument(
        "--g", action="store_true", help="Generate graphs if this flag is set"
    )
    parser.add_argument(
        "-j",
        "--json",
        action="store_true",
        help="Output results in JSON format if this flag is set",
    )
...
mcu_df = pd.read_csv("CapSimu/mcu.csv")
...
time = (
    np.arange(len(V_max_values)) / 1000
...
def simulate_for_capacitor(C):
    ...
    V_check_thres = np.sqrt(
        2 * (total_checkpoint_energy + (0.5 * C * V_cutoff**2)) / C
    )  # checkpoint voltage threshold
    charging_states = []
    ...
    for i, t in enumerate(time):
        ...
        if charging:
            ...
            if V_max > V:
                V = V_initial + (V_max - V_initial) * (
                    1 - np.exp(-(0.001) / (R_charging * C))
                )
                V_initial = V
            if V >= V_discharge_start:
                ...
                energy_discharge_event_occurred = (
                    False  # Reset the flag at the beginning of discharge
                )
        else:
            ...
            if V <= V_check_thres and not energy_discharge_event_occurred:
                charging_states.append(
                    t
                )  # Record time slightly before the new charge cycle
                energy_discharge_event_occurred = (
                    True  # Set the flag to prevent multiple recordings
                )
            ...
            if V <= V_cutoff:
                ...
                total_energy_consumed += (
                    E_initial - E_final - total_checkpoint_energy - total_restore_energy
                )
    ...
    if args.json:
        results = {
            ...
            "Total energy required": float(
                total_cycles_required * energy_per_cycle
            ),  # Convert to float
            ...
            "Execution completed": bool(
                total_energy_consumed >= total_cycles_required * energy_per_cycle
            ),  # Convert to bool
        }
        # print(json.dumps(results, indent=4))
        with open(f"logs/results.json", "w") as f:
            json.dump(results, f)
    else:
        ...
        print(
            f"Total energy required: {total_cycles_required * energy_per_cycle} Joules"
        )
        if total_energy_consumed >= total_cycles_required * energy_per_cycle:
    ...
    if args.g:
        ...
        plt.plot(
            ...
            color="#B9B9B9",
            linestyle="--",
        )  # Plot up to the break point
        plt.plot(
            time[: i + 1],
            voltage[: i + 1],
            label=f"Capacitor Voltage (C = {C} F)",
            color="#000000",
        )  # Plot up to the break point
